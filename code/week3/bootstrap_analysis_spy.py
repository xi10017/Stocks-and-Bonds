"""
Block Bootstrap Analysis - Week 3 (SPY 2002-2025)

Uses optimal parameters found from parameter tuning:
- Lookback: 15 months (315 days)
- Percentile Cutoff: 70%
"""
...
from utils import download_data, calculate_sharpe_ratio, calculate_progressive_bond_allocation, calculate_progressive_strategy_returns
...
# OPTIMAL PARAMETERS FOR SPY (2002-2025)
# These should match the best parameters from parameter_tuning_spy.py
LOOKBACK_WINDOW = 21 * 15  # 15 months
PERCENTILE_CUTOFF = 0.60  # 70%
SCALING = 'sigmoid'  # Options: 'linear', 'sigmoid', 'exponential', 'power'
...
# =============================================================================
print("=" * 70)
print("BLOCK BOOTSTRAP ANALYSIS - WEEK 3 (SPY 2002-2025)")
print("=" * 70)
print(f"\nDownloading data...")
...
# Generate VIX strategy signals with progressive allocation
...
print(f"Optimal parameters: Lookback={LOOKBACK_WINDOW} days (15mo), Cutoff={PERCENTILE_CUTOFF*100:.0f}%, Scaling={SCALING}")
print(f"Average bond allocation: {df['Bond_Allocation'].mean():.1%}")
...
original_sharpe = {
    "S&P 500 (SPY)": calculate_sharpe_ratio(df["Stock_Return"], RISK_FREE_RATE),
...
bootstrap_results = {}
for name, returns in [("S&P 500 (SPY)", df["Stock_Return"]), 
...
for name in ["S&P 500 (SPY)", "10Y Treasury", "VIX Strategy"]:
...
sp500_samples = bootstrap_results["S&P 500 (SPY)"]
...
sp500_ci = (ci_results["S&P 500 (SPY)"]["ci_lower"], ci_results["S&P 500 (SPY)"]["ci_upper"])
...
colors = {"S&P 500 (SPY)": "#1f77b4", "10Y Treasury": "#2ca02c", "VIX Strategy": "#d62728"}
...
ax1.set_title("Bootstrap Distribution of Sharpe Ratios (SPY 2002-2025)", fontsize=13, fontweight='bold')
ax1.legend()
ax1.grid(True, alpha=0.3)

# Plot 2: Box plot
ax2 = axes[0, 1]
bp_data = [bootstrap_results["S&P 500 (SPY)"], bootstrap_results["10Y Treasury"], bootstrap_results["VIX Strategy"]]
bp = ax2.boxplot(bp_data, labels=["S&P 500\n(SPY)", "10Y Treasury", "VIX Strategy"], patch_artist=True)
for patch, color in zip(bp['boxes'], [colors["S&P 500 (SPY)"], colors["10Y Treasury"], colors["VIX Strategy"]]):
...
ax3 = axes[1, 0]
strategies = ["S&P 500 (SPY)", "10Y Treasury", "VIX Strategy"]
...
plt.tight_layout()
plt.savefig("../../results/week3/bootstrap_analysis.png", dpi=150, bbox_inches="tight")
print(f"\n✓ Chart saved to 'results/week3/bootstrap_analysis.png'")
...
summary_df.to_csv("../../data/week3/bootstrap_results.csv", index=False)
print(f"✓ Results saved to 'data/week3/bootstrap_results.csv'")

# =============================================================================
# Print Summary
# =============================================================================
print(f"\n{'='*70}")
print("SUMMARY - WEEK 3 (SPY 2002-2025)")
print(f"{'='*70}")
print(f"""
Data Period: {df.index[0].date()} to {df.index[-1].date()} (~{len(df)/252:.1f} years)
Optimal Parameters: 15mo lookback, 70% cutoff
...
S&P 500 (SPY):
  - Sharpe Ratio: {original_sharpe['S&P 500 (SPY)']:.3f}
  - 95% CI: [{ci_results['S&P 500 (SPY)']['ci_lower']:.3f}, {ci_results['S&P 500 (SPY)']['ci_upper']:.3f}]
